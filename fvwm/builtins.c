	      ...
	      unsigned long context, char *action, int *Module)
{
  ...
  if (DeferExecution
      (eventp, &w, &tmp_win, &context, SELECT, ButtonRelease)) return;
...
MenuRoot *FindPopup(char *action)
{
  ...
  while (mr != NULL) {
	...
	if (GetTwoArguments(action, &(mr->x), &(mr->y), &dummy, &dummy) ==
	    2) mr->has_coords = True;
		    ...
		    unsigned long context, char *action, int *Module)
{
  ...
  if (LookInList
      (Scr.TheList, tmp_win->realname, &tmp_win->class, &junk, &junk,
       &junkD, &junkD, &junkD, &junkC, &junkC, &junkN, BoxJunk,
       &method) & STAYSONTOP_FLAG) tmp_win->flags |= ONTOP;
  KeepOnTop();
}
...
{
  if (DeferExecution
      (eventp, &w, &tmp_win, &context, DESTROY, ButtonRelease)) return;
		 ...
		 unsigned long context, char *action, int *Module)
{
  ...
  if ((val1_unit != Scr.MyDisplayWidth) ||
      (val2_unit != Scr.MyDisplayHeight)) {
    fvwm_msg(ERR, "SetDeskSize",
	     "DeskTopSize arguments should be unitless");
  }
  ...
  if (Scr.VyMax < 0)
    Scr.VyMax = 0;
  Broadcast(M_NEW_PAGE, 5, Scr.Vx, Scr.Vy, Scr.CurrentDesk, Scr.VxMax,
	    Scr.VyMax, 0, 0);
		   ...
		   unsigned long context, char *action, int *Module)
{
  XGCValues gcv;
  unsigned long gcm;
  char *menufore = NULL, *menuback = NULL, *selfore = NULL, *selback =
      NULL;
  ...
  if (Scr.d_depth > 2) {
    ...
    if (selfore != NULL)
      Scr.SelColors.fore = GetColor(selfore);
    if (selback != NULL)
      Scr.SelColors.back = GetColor(selback);
  ...
  gcm =
      ...
      GCForeground | GCBackground | GCFillStyle | GCFont;
  gcv.function = GXcopy;
  gcv.plane_mask = AllPlanes;
		...
		unsigned long context, char *action, int *Module)
{
  ...
  if (Scr.d_depth > 2) {
    if (hifore != NULL)
      Scr.ActiveTitleColors.fore = GetColor(hifore);
    if (hiback != NULL)
      Scr.ActiveTitleColors.back = GetColor(hiback);
  } else {
		 ...
		 unsigned long context, char *action, int *Module)
{
  ...
  if (Scr.d_depth > 2) {
    if (winfore != NULL)
      Scr.WinColors.fore = GetColor(winfore);
    if (winback != NULL)
      Scr.WinColors.back = GetColor(winback);
    if (stdfore != NULL)
      Scr.TitleColors.fore = GetColor(stdfore);
    if (stdback != NULL)
      Scr.TitleColors.back = GetColor(stdback);
		    ...
		    unsigned long context, char *action, int *Module)
{
  ...
  if (Scr.d_depth > 2) {
    if (fore != NULL)
      Scr.StickyColors.fore = GetColor(fore);
    if (back != NULL)
      Scr.StickyColors.back = GetColor(back);
  } else {
		 ...
		 unsigned long context, char *action, int *Module)
{
  ...
  if (Scr.SelGC != NULL)
    XChangeGC(dpy, Scr.SelGC, gcm, &gcv);
  if (Scr.MenuReliefGC != NULL)
		    ...
		    unsigned long context, char *action, int *Module)
{
  ...
  if ((newfont = GetFontOrFixed(dpy, font)) != NULL) {
    ...
    Scr.TitleHeight =
	Scr.WindowFont.font->ascent + Scr.WindowFont.font->descent + 3;
    extra_height -= Scr.TitleHeight;
    tmp = Scr.FvwmRoot.next;
    hi = Scr.Hilite;
    while (tmp != NULL) {
...
/*
  ...
  if(Scr.button_pmask[button] != None)
    XFreePixmap(dpy, Scr.button_mask[button]);
*/

  if (XpmReadFileToPixmap(dpy, Scr.Root, path,
			  &Scr.button_pixmap[button],
			  &junkmaskPixmap, &xpm_attributes) != XpmSuccess) {
    fvwm_msg(ERR, "ButtonStyle", "Couldn't load XPM file");
    free(path);
    return;
  }

  if ((xpm_attributes.width != 13) || (xpm_attributes.height != 11)) {
...
FvwmWindow *Circulate(char *action, int Direction, char **restofline)
{
  ...
  char *t, *tstart, *name =
      NULL, *expression, *condition, *prev_condition = NULL;
  char *orig_expr;
  Bool needsIconic = 0;
  Bool needsNormal = 0;
