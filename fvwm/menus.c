 ...
 *
 ***********************************************************************/
void RelieveRectangle(Window win, int x, int y, int w, int h, GC Hilite,
		      GC Shadow)
{
...
void PaintEntry(MenuRoot * mr, MenuItem * mi)
{
  ...
  if (mi->func_type == F_TITLE) {
    ...
    if (mi->next != NULL) {
      DrawSeparator(mr->w, ShadowGC, ReliefGC, 5 + mr->xoffset,
  ...
  if (mi->func_type == F_NOP && *mi->item == 0) {
    DrawSeparator(mr->w, ShadowGC, ReliefGC, 2 + mr->xoffset,
		  ...
		  mr->width - 3, y_offset - 1 + (HEIGHT_SEPARATOR >> 1),
		  0);
  }
  ...
  if (*mi->item)
	...
	XDrawString(dpy, mr->w, ReliefGC,
		    mi->x + mr->xoffset + 1, text_y + 1, mi->item,
		    mi->strlen);
      XDrawString(dpy, mr->w, ShadowGC, mi->x + mr->xoffset, text_y,
		  mi->item, mi->strlen);
    }
  ...
  if (mi->strlen2 > 0)
    if (check_allowed_function(mi)) {
      ...
      if (!mi->state)
	XDrawString(dpy, mr->w, ReliefGC,
		    mi->x2 + 1 + mr->xoffset, text_y + 1, mi->item2,
		    mi->strlen2);
      XDrawString(dpy, mr->w, ShadowGC, mi->x2 + mr->xoffset, text_y,
		  mi->item2, mi->strlen2);
    }
  ...
  if (mi->hotkey < 0)
    if (check_allowed_function(mi)) {
      ...
      /* should be a shaded out underline, no just re-colored. */
      if (!mi->state)
	DrawUnderline(mr->w, ReliefGC, mi->x2 + 1 + mr->xoffset,
		      text_y + 1, mi->item2, -1 - mi->hotkey);
 ...
 *
 ****************************************************************************/
void DrawSeparator(Window w, GC TopGC, GC BottomGC, int x1, int y1, int x2,
...
Bool PopUpMenu(MenuRoot * menu, int x, int y)
{
  ...
  if ((Tmp_win) && (menu_on == 1) && (Context & C_RALL)) {
	...
	ButtonPosition(Context,
		       Tmp_win) * Tmp_win->title_height - menu->width + 1;
  }
