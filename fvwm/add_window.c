...
FvwmWindow *AddWindow(Window w)
{
   ...
   *  dont decorate it.
   *  If it is a transient, and DecorateTransients was specified,
   *  decorate anyway
   */
  ...
  if (!PlaceWindow(tmp_win, tflag, Desk)) return NULL;
...
*/
  ...
  /*
      RBW - 05/15/1998 - add it into the stacking order chain also.
  ...
  */
  ...
  /* That's not all, we'll double-reparent the window! */
  attributes.cursor = Scr.FvwmCursors[DEFAULT];
  ...
  if (tmp_win->flags & TITLE) {
	...
	XCreateWindow(dpy, tmp_win->frame, tmp_win->title_x,
		      ...
		      InputOutput, CopyFromParent, t_valuemask,
		      &t_attributes);
  ...
  if (tmp_win->title_icon != NULL)
    Broadcast(M_MINI_ICON, 6, tmp_win->w,	/* Watch Out! I reduced the set of infos... */
	      tmp_win->title_icon->picture,
	      tmp_win->title_icon->mask,
	      tmp_win->title_icon->width,
	      tmp_win->title_icon->height, tmp_win->title_icon->depth, 0);
...
{
  unsigned long nitems, bytes_remain, flags = 0L;
  Atom atype, *ap, *protocols = NULL;
  int  i, n;
  int  aformat;
...
unsigned long LookInList(name_list *list, char *name, XClassHint *class,
			 char **value, char **tvalue, int *Desk,
			 int *border_width, int *resize_width,
