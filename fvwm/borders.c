	       ...
	       Window expose_win)
{
  ...
  if (t->flags & TITLE) {
	...
	if (flush_expose(*win) || (expose_win == *win) ||
	  ...
	  if ((i == 0) && (t->title_icon)) {	/* title icon present? */
	    ...
	    RelieveButton(t, *win, 0, 0,
			  ...
			  ReliefGC, ShadowGC, (PressedW == *win));
#if 0
	    XCopyArea(dpy, Scr.button_pixmap[i << 1], *win,
		      ...
		      (PressedW == *win) ? 2 : 1);
#else
	    ChangeWindowColor(*win, BackColor);

	    gcm = GCClipMask | GCClipXOrigin | GCClipYOrigin;
	    gcv.clip_mask = Scr.button_mask[i << 1];
	    gcv.clip_x_origin = (PressedW == *win) ? 2 : 1;
	    gcv.clip_y_origin = (PressedW == *win) ? 2 : 1;
	    XChangeGC(dpy, Scr.ScratchGC3, gcm, &gcv);
    ...
    for (i = 0, win = t->right_w; i < Scr.nr_right_buttons; i++, win++) {
      if (*win != None) {
	//ChangeWindowColor(*win, /*Title*/BackColor);
	if (flush_expose(*win) || (expose_win == *win) ||
	    (expose_win == None)) {
	  Pixmap pix, mask;
	  ...
	  if ((i == 1) && (t->flags & MAXIMIZED)) {
	    pix = Scr.button_pixmap[(i << 1)];
	    mask = Scr.button_mask[(i << 1)];
	  } else {
	    pix = Scr.button_pixmap[(i << 1) + 1];
	    mask = Scr.button_mask[(i << 1) + 1];
	  }
#if 0
  ...
  if (t->flags & BORDER) {
    ...
    for (i = 0, side = t->sides, corner = t->corners; i < 4;
	 i++, side++, corner++) {
	...
	RelieveWindow(t, *corner, 0, 0, t->corner_width,
		      ((i >> 1) ? t->corner_width +
		       t->bw : t->corner_width), ReliefGC, ShadowGC,
		      corners[i]);
      }
    }
...
void SetTitleBar(FvwmWindow *t, Bool onoroff, Bool NewTitle)
{
  ...
  if (Scr.d_depth < 2) {
    XFillRectangle(dpy, t->title_w,
		   Scr.BlackGC, x - 2, 0, w + 4, t->title_height);
...
*/
  ...
  if (Resized) {
    ...
    tmp_win->title_width =
	w - (tmp_win->boundary_width << 1) + tmp_win->bw;
	...
	if (*win != None) {
      ...
      for (i = 0, win = tmp_win->right_w; i < Scr.nr_right_buttons;
	   i++, win++) {
	if (*win != None) {
	  xwc.x -= Scr.button_width;
    ...
    if (tmp_win->flags & BORDER) {
      ...
      if (w < (tmp_win->corner_width << 1))
	tmp_win->corner_width = w / 3;
      if (h < (tmp_win->corner_width << 1))
