...
void CreateIconWindow(FvwmWindow * tmp_win, int def_x, int def_y)
{
  ...
  if (!(tmp_win->flags & NOICON_TITLE) || (tmp_win->icon_p_height == 0))
    tmp_win->icon_w =
	XCreateWindow(dpy, Scr.Root, final_x,
  ...
  if ((tmp_win->flags & ICON_OURS) && (tmp_win->icon_p_width > 0) &&
	...
	XCreateWindow(dpy, Scr.Root, final_x, final_y,
		      tmp_win->icon_p_width, tmp_win->icon_p_height, 0,
		      CopyFromParent, CopyFromParent, CopyFromParent,
		      valuemask, &attributes);
  } else {
    attributes.event_mask = (ButtonPressMask | ButtonReleaseMask |
...
void AutoPlace(FvwmWindow * t)
{
  ...
  if ((t->flags & StickyIcon) || (t->flags & STICKY)) {
    ...
    base_x =
	((t->frame_x + Scr.Vx + (t->frame_width >> 1)) /
	 Scr.MyDisplayWidth) * Scr.MyDisplayWidth - Scr.Vx;
    base_y =
	((t->frame_y + Scr.Vy + (t->frame_height >> 1)) /
...
void DeIconify(FvwmWindow * tmp_win)
{
  ...
  for (t = Scr.FvwmRoot.next; t != NULL; t = t->next) {
	...
	XUnmapWindow(dpy, t->icon_pixmap_w);
      Broadcast(M_DEICONIFY, 3, t->w, t->frame, (unsigned long) t, 0, 0, 0,
		0);
    }
  }
